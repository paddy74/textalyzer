 ...
 * @param stopwordSet
 */
void StopwordFilter::removeWords(
    std::vector<std::string> & strVect,
    std::unordered_set<std::string> const & stopwordSet
)
{ strVect = StopwordFilter::getSymDiff(strVect, stopwordSet); }
...
{ StopwordFilter::removeWords(strVect, StopwordFilter::getStopwords(language)); }
 ...
 * @param strVect
 */
void StopwordFilter::removeWords(std::vector<std::string> & strVect)
{ StopwordFilter::removeWords(strVect, StopwordFilter::DEFAULT_LANGUAGE); }


/* Getters */

/**
 * @brief Get the `stopwordSet` of the given language.
 *  If a language is not found, the default is provided.
 *
 * @param language
 * @return std::unordered_set<std::string> const&
 */
std::unordered_set<std::string> const & StopwordFilter::getStopwords(
    std::string const & language)
{
    if (language == "english")
        return StopwordFilter::englishStopwords;
...
/**
 * @brief Identifies if a given string is a stopword or is empty.
 *
 * @param str
 * @param stopwordSet
...
{
    return (str.size() < 2U)
        || (stopwordSet.find(str) != stopwordSet.end());
}
 ...
 */
template<typename T>
std::vector<T> StopwordFilter::getSymDiff(
